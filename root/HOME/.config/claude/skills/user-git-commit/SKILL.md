...
1. Sync: `$ git-sync-onto-main.zsh`. Exit 22 → resolve conflicts. Exit 23 → continue on `main`
2. Decide amend/fresh commit:
   ...
   - else → fresh commit
3. Stage all: `$ git add .`
4. Generate from staged diff: `$ llm-git-commit-msg-suggest.zsh` → `{subject, description}`.
