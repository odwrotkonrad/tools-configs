...
1. Sync: `$ s-root-git-sync-onto-main`. Exit 22 → resolve conflicts. Exit 23 → continue on `main`
2. Decide amend/fresh commit:
   ...
   - else → fresh commit
3. Stage all: `$ git add .`
4. Generate from staged diff: `$ s-root-llm-git-commit-msg-suggest` → `{subject, description}`.
