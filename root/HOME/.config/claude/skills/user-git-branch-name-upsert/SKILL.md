...
1. Sync onto main: `$ s-root-git-sync-onto-main`. Exit 22 (conflicts) → resolve conflicts. Exit 23 (merged, now on main) → continue
2. Generate: `$ s-root-llm-git-branch-name-suggest` → `{name}`
