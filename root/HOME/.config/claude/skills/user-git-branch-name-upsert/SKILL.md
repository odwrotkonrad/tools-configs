...
1. Sync onto main: `$ git-sync-onto-main.zsh`. Exit 22 (conflicts) → resolve conflicts. Exit 23 (merged, now on main) → continue
2. Generate: `$ llm-git-branch-name-suggest.zsh` → `{name}`
