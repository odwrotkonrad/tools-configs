...
##[>] files
#>[what] ls - list directory contents
...
###[>] rg
#>[what] rg - recursively search the current directory for lines matching a pattern
...
#[what] rg wrapper limiting column width to terminal size
#[why] to set --max-columns dynamically based on current terminal width
...
alias a-rt-search-files-no='fn-root-rg --files' #[what] instead of searching list files that would be searched
...
##[>] manual pages
#>[what] man - display online manual documentation pages
...
#>[what] pstree – list processes as a tree
...
##[>] claude >[what] agentic coding tool
...
#[why] it happens claude tries to ask questions in dontAsk mode
...
##[>] other
#>[what] fc - zsh builtin, controls the interactive history mechanism
#  -l - list to stdout
#  -n - do not show event numbers
#/[what]
alias a-rt-history-file-contents-show="fc -ln 1"   #[what] starting from event no 1

alias a-rt-keystrokes-listen-output-raw="kitten show-key -m kitty" #[what] start listening for keystrokes to output raw sequences, useful for building keybidnings

#[what] ssh - OpenSSH remote login client, -G print configuration
alias a-rt-ssh-show-config="ssh -G localhost"

#>[what] prometheus - monitoring system & time series database
