...
3. `printf '%s' "<additional-runtime-instructions>" | s-rt-llm-git-commit-msg-suggest` → JSON `{subject, description}`.
4. Show the message, then commit it: `git commit -m "<subject>" -m "<description>"`.
