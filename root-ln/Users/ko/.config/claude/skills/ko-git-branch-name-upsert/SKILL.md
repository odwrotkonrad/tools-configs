...
## /ko-git-branch-name-upsert Steps

1. Sync onto main: `$ s-rt-git-sync-onto-main`. Exit 22 (conflicts) → resolve conflicts. Exit 23 (merged, now on main) → continue
2. Generate: `$ s-rt-llm-git-branch-name-suggest` → `{name}`
