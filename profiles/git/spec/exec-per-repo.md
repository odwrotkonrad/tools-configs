...
Scenario: interactive progress dashboard refreshes in place
  ...
  And each repo renders as a bold `### <repo> <✅|❌|🕐> <clock> (<pid>)` block: `log: <log file>`, then `tail: > <most recent non-empty log line>` on one line (last 15 lines scanned bottom-up, CR/ANSI stripped, width-truncated, `tail: >` when nothing non-empty), so block heights stay fixed across redraws
