...
Scenario: --main reports the latest main pipeline
  ...
  And with no main pipeline `## Pipeline Status` prints `none`, exits 1

Scenario: pipeline verdict is the exit code
  Status: implemented
  When the report finishes
  Then it exits 1 when the pipeline errored (failed or canceled) or no pipeline exists (`none`), so multi-repo runs flag it ❌
  And exits 0 on any other status: success, no open MRs, still running via --no-wait, manual/blocked
