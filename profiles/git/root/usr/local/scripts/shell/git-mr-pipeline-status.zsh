...
#   Exit 1 only on an errored pipeline (failed | canceled); success,
#   still-running (--no-wait), manual/blocked, and no pipeline (logged
#   "none: <reason>") exit 0.
...
if [[ $target_branch == main ]] { main_flag=1; target_branch= }
if (( main_flag )) && [[ -n $target_branch ]] {
  print -ru2 -- "${0:t}: --main excludes --branch"; exit 2
}
: ${target_branch:=$(git rev-parse --abbrev-ref HEAD)}
if [[ $target_branch == main ]] main_flag=1
...
repo_section() {
  ...
  if (( mr_count <= 1 )) {
    ...
    print -r -- $'\n'"$bold### ⚠️  Open MR count: $mr_count$reset"
    jq -r --arg branch $target_branch \
      'sort_by(.updated_at) | reverse
      | map(select(.source_branch == $branch)) + map(select(.source_branch != $branch))
      | to_entries[]
      | (if .key > 0 then "" else empty end),
        "name: \(.value.title)", "url: \(.value.web_url)", "update-time: \(.value.updated_at)",
        (if .value.source_branch == $branch then "current: true" else empty end)' <<< $mrs_json
  }
}
...
if (( main_flag )) {
  ...
  pipeline_id=$(jq -r '.id // empty' <<< $main_json)
  print -r -- "$bold# Main Pipeline$reset"
  if [[ -z $pipeline_id ]] { print -r -- $'\n'"$bold## Pipeline Status$reset"$'\n'"none: no push-sourced pipeline on main (repo may not run main pipelines)"; exit 0 }
  jq -r '"url: \(.web_url)", "sha: \(.sha[0:8])"' <<< $main_json
  repo_section
} else {
  if (( mr_count == 0 )) { print -r -- "no open MRs"; exit 0 }

  mr_iid=$(jq -r --arg branch $target_branch \
    '. as $all | [$all[] | select(.source_branch == $branch)]
    | if length > 0 then .[0].iid else ($all | sort_by(.updated_at) | last | .iid) end' <<< $mrs_json)
  mr_json=$(glab mr view $mr_iid --output json)
  jq -r --arg bold $bold --arg reset $reset \
    '"\($bold)# MR: !\(.iid)\($reset)", "name: \(.title)", "url: \(.web_url)", "pipeline-url: \(.head_pipeline.web_url // "none")"' <<< $mr_json
  ...
  read files insertions deletions <<< $(git diff --numstat origin/main...$ref | awk '{f++; a+=$1; d+=$2} END {print f+0, a+0, d+0}')
  print -r -- "line changes: +$insertions -$deletions (~$files files)"
  print -r -- "commit count: $(git rev-list --count origin/main..$ref)"

  pipeline_id=$(jq -r '.head_pipeline.id // empty' <<< $mr_json)
  if [[ -z $pipeline_id ]] { print -r -- $'\n'"$bold## Pipeline Status$reset"$'\n'"none: MR !$mr_iid has no pipeline"; exit 0 }
}

jq_defs='def emoji: {success:"✅",failed:"❌",canceled:"🚫",skipped:"⏭️ ",manual:"⚙️ ",running:"🕐"}[.] // "⏳";
...
pipeline_json=$(glab api projects/:id/pipelines/$pipeline_id)
if (( wait_flag )) {
  typeset -A job_reported
  log_started=0
  while [[ $(jq -r .status <<< $pipeline_json) == (created|preparing|pending|waiting_for_resource|running) ]] {
    if (( ! log_started )) { log_started=1; print -ru2 -- $'\n'"$bold## Inprogress Log$reset" }
    jobs_json=$(glab api "projects/:id/pipelines/$pipeline_id/jobs?per_page=100")
    for line (${(f)"$(jq -r "$jq_defs"'sort_by(.id)[]
        | select(.status | IN("success","failed","canceled","skipped"))
        | "\(.id)\t\(.status | emoji) \(job_dur | dur) \(.name)"' <<< $jobs_json)"}) {
      job_id=${line%%$'\t'*}
...
glab api "projects/:id/pipelines/$pipeline_id/jobs?per_page=100" |
  jq -r --arg bold $bold --arg reset $reset "$jq_defs"'sort_by(.id) | . as $jobs
    | "", "\($bold)## Stages\($reset)", "",
      ((map(.stage) | reduce .[] as $s ([]; if index($s) == null then . + [$s] else . end))[] as $stage
        | ($jobs | map(select(.stage == $stage))) as $stage_jobs
        | "\($bold)### \($stage)\($stage_jobs | stage_dur | if . == null then "" else " \(dur | gsub("^ +| +$"; ""))" end)\($reset)",
          ($stage_jobs[] | "\(.status | emoji) \(job_dur | dur) \(.name)\({manual:" (manual trigger)",canceled:" (canceled)"}[.status] // "")", "url: \(.web_url)", ""))'

jq -r --arg bold $bold --arg reset $reset "$jq_defs"'
  "\($bold)## Pipeline Status\($reset)", "\(.status | emoji) \(if .status == "success" then "" else "\(.status) " end)\(.duration | dur)"' <<< $pipeline_json
[[ $(jq -r .status <<< $pipeline_json) == (failed|canceled) ]] && exit 1
exit 0
##[<] 🤖🤖
