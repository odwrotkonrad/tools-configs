...
function install_kubectx {
  ...
  for bin ( kubectx kubens ) {
    curl --connect-timeout 30 --retry 10 --retry-delay 30 --retry-all-errors -fsSL "https://github.com/ahmetb/kubectx/releases/download/v${version}/${bin}_v${version}_${os}_${arch}.tar.gz" | tar -xzf - -C $tmp $bin
