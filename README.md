<!-- autogenerated using git::gitlab.com/konradodwrot/cross-repo/prose/assets@v0.0.69//repos/tools-configs/templates/3-audience/README.md.ontoRepo.tpl -->
...
Git-tracked dotfiles extended into root OS space: every option explicit, plus scripts and an observability stack. Che loads one `root/` tree onto the host: symlinked by default, `.ontoHost.cp` copied, `*.ontoHost.tpl` rendered. Repo docs and the license render from `cross-repo/prose/assets` at a pinned version. The AI toolchain lives apart, in `ai-harness/ai-tools-configs`, included at `AI_TOOLS_CONFIGS_REF`. Only the local `.env` seed renders from a repo-local template.
...
- No AI inside: the harness releases on its own cadence, behind `AI_TOOLS_CONFIGS_REF`.
