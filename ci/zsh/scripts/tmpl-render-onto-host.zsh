...
function backup_dest {
  local dest=$1
  if [[ $dest == $HOME/* ]] {
    fn-backup-before-overwrite --repo-root $configs $dest
  } elif [[ -e $dest && ! -L $dest ]] {
    sudo cp -p -- $dest $dest.$(date +%Y%m%dT%H%M%S).bk ; fn-log-msg -t backup -- $dest.bk
  }
}
...
function render_template {
  ...
  if [[ $(head -1 $template) != '---' ]] {
    ...
    rm -f $tmp ; fn-log-msg -t gomplate -- $render_to
    return
  }
  ...
  for render_to ( $render_tos ) {
    ...
    fn-log-msg -t gomplate -- $render_to
  }
  rm -f $body
  ##[<] 🤖🤖
}
