...
for dir ( ${(o)repo_dirs_all} ) {  #[what] 🤖🤖 (o) sort -> parents before children
  ...
  if [[ $dir == HOME/* ]] {  #[what] 🤖🤖 HOME-tree: made as the user (user-owned, no chown), 0750 (group read, no world)
    ...
    (umask 022; mkdir $ddir); fn-log-msg -t mkdir -- $ddir  #[what] 🤖🤖 root-tree: 0755 (world-readable); no -p
  }
}
...
for file ( $repo_files_ex ) {
  dest=$(to_dest $file)
  if [[ ${dest:A} != ${file:A} ]] {
    fn-backup-before-overwrite --repo-root $configs $dest  #[what] keep any non-repo dest first
...
for file ( $copy_files ) {
  ...
  if { ! cmp -s ${file:a} $dest } {
    fn-backup-before-overwrite $dest  #[what] a real copied dest is never a repo-pointing symlink (no --repo-root)
